...
public class Predator extends Species {
    ...
    public Predator(String name, int energyConsumption, int mortality, Map map, Random seed, int i, int j) {
    ...
    public int[] observeFromCenter(int radius) { //1 3 , 2 5 , 3 7,  4 9
        ...
        for (int m = i - radius; m < i + radius; m++) {
            if (map.ifHavePrey(m, j - radius, "level1")) {
                array[0] = m;
                array[1] = j - radius;
                energy += 30;
                return array;
        ...
        for (int p = j - radius; p < j + radius; p++) {
            if (map.ifHavePrey(i - radius, p, "level1")) {
                array[0] = i - radius;
                array[1] = p;
                energy += 30;
                return array;
    ...
    public void fertile(int len, int width) {
        if (mortality <= maxMortality && energy > 2 * energyConsumption && energy - maxEnergy >= 0) {
            Predator newSp = new Predator(name, energyConsumption, 5 * energyConsumption, map, seed, len, width);
